  ...
  return (
    ...
    <Card sx={{ maxWidth: 300, borderRadius: 3 }}>
      <CardMedia
        ...
        image={image}
        alt={title}
        sx={{ width: '100%', objectFit: 'cover', borderRadius: '3px 3px 0 0' }}
      />
      <CardContent>
        <Typography
        ...
        <div className="CardEventDetail">
          <Typography
            fontFamily={'Yekan, sans-serif'}
            sx={{ fontSize: 17, fontWeight: 700, marginBottom: 1 }}
            variant="h6"
          ...
          >
            {Number(discount) ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <span>{usePersianNumber(price * (100 - Number(discount)) / 100)}</span>
                <span style={{ fontSize: 12 }}>هزارتومان</span>
                <span
                  style={{
                    opacity: 0.5,
                    fontSize: 16,
                ...
                >
                  {usePersianNumber(discount)}%
                </span>
              </div>
            ) : (
              <span>{usePersianNumber(price)} هزارتومان</span>
            )}
          </Typography>
        </div>
      </CardContent>
      ...
      >
        <CardActions disableSpacing sx={{ justifyContent: 'center' }}>
          ...
          <Typography fontFamily={'Yekan, sans-serif'} sx={{ fontSize: 15 }} variant="h1" color="text.secondary">
            <p style={{ fontSize: 15 }}>
              {usePersianNumber(date.split(' ')[0])}/{usePersianNumber(date.split(' ')[1])}/
              {usePersianNumber(date.split(' ')[2])}
            </p>
          </Typography>
          <IconButton aria-label="time">
            <BiTime />
          ...
          <Typography fontFamily={'Yekan, sans-serif'} sx={{ fontSize: 15 }} variant="h1" color="text.secondary">
            <p style={{ fontSize: 15 }}>
              {usePersianNumber(time.split(' ')[0])}:{usePersianNumber(time.split(' ')[1])}:
              {usePersianNumber(time.split(' ')[2])}
            </p>
          </Typography>
        </CardActions>
      </div>
